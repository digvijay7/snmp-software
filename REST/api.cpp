...
bool api::authenticateAPI(const string& url, const map<string, string>& argvals, string& response, bool & is_sudo, char* ipAddress)
{
  ...
  if(url == "/register" || url == "/deregister"){
	try{
		...
		std::stringstream ss;
		ss <<  "SELECT count(*) FROM uid WHERE ip ='" <<ipAddress<<"';";
		pqxx::work w(conn);
...
bool api::executeAPI(const string& url, const map<string, string>& argvals, string& response,bool & is_sudo, char* ipAddress){
  ...
  return _executeAPI(url, params, type, response,is_sudo,  ipAddress);
}

bool api::_executeAPI(const string& url, const struct args_container & argvals, 
        Executor::outputType type, string& response,bool & is_sudo, char* ipAddress)
{
  bool ret = false;
  if(_executor.get_type() == VALID_API_UID){
    ...
    ret = _executor.loc_deregister(argvals,type,response,url, ipAddress);
  }
  return ret;
}
